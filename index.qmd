...
VAEs work by compressing data into a compact representation (the "latent space"), then reconstructing it. They use a technique called ELBO (Evidence Lower Bound) to make this work efficiently—it's a workaround that approximates what we really want to measure.

- **What they're good at:** Learning useful representations of data, generating images, and handling cases where you want some control over what gets generated.
- **How they generate:** Sample a random point in the latent space and decode it into an output. (Fast)
- **Examples:** β-VAE, Conditional VAE
> [Go to VAE Models →](vae.qmd)

### Autoregressive Models

These models generate content one piece at a time, like writing a sentence word by word. Each new piece depends on everything that came before it. GPT is the most famous example—it predicts the next token based on all previous tokens.

**What they're good at:** Language modeling, code generation, anything sequential. They're exact—they can tell you precisely how likely any sequence is.

**The tradeoff:** You can't parallelize it because each step depends on the previous one. (Slow generation)

**Examples:** GPT, PixelRNN

### Normalizing Flows

Normalizing Flows transform simple random noise into complex data through a series of reversible steps. Think of it as a recipe you can run backwards—start with noise, apply transformations, get realistic data. Because everything is reversible, you can compute exact probabilities.

**What they're good at:** Density estimation and scenarios where you need exact likelihoods.

**How they generate:** Sample from simple noise and run it through the transformations. (Fast)
...
EBMs assign an "energy level" to every possible input. Real data gets low energy, fake data gets high energy. The model learns what energy landscape makes sense. The catch is that computing exact probabilities is intractable—the math doesn't work out cleanly.

**What they're good at:** Modeling uncertainty and building compositional systems where you can combine different concepts.

**The tradeoff:** Very slow sampling. You need techniques like MCMC or Langevin dynamics, which require many iterations.

**Examples:** Score-based EBMs

> [Go to EBMs →](ebm.qmd)

### Generative Adversarial Networks (GANs)

GANs use a clever trick: pit two networks against each other. One network (the generator) tries to create fakes. The other (the discriminator) tries to spot them. They compete until the fakes become so good the discriminator can't tell the difference.

**What they're good at:** Creating high-quality images and style transfer. They don't model probabilities explicitly—they just learn to generate convincing outputs.

**How they generate:** Sample random noise and pass it through the generator. (Fast)

**Examples:** StyleGAN, CycleGAN

### Diffusion Models

Diffusion models work backwards from noise. Start with pure noise, then gradually remove it step by step until you reveal a clean image. It's like watching a photo develop in reverse. The model learns how to denoise at each step.

**What they're good at:** High-resolution images and audio. They've become incredibly popular because they produce excellent results and are more stable to train than GANs.

**The tradeoff:** Slow. You need hundreds of steps to generate a single sample.
