...
Training means adjusting those settings until the model's version matches the real thing. The model keeps tweaking itself to minimize the gap between what it generates and what actually exists. Once it's trained, you can generate new content by sampling from what the model learned.
