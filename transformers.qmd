...
Before the attention mechanism can operate, each input token is mapped to a continuous vector representation and enriched with positional information.

##### Token Embedding
Each token in the sequence is mapped to a fixed-dimensional vector using an embedding matrix:
$$
X_\text{tokens} \in \mathbb{R}^{n \times d_e}
$$
where:
...
##### Positional Encoding
Since attention is permutation-invariant and does not capture token order by itself, we add **positional encoding** to the embeddings:
$$
