...
![Feedforward Network in Transformer](images/feedforward_network.png)

Here the FFN (pink box) sits on top of each token's output after self-attention, applying the same MLP to each token embedding in parallel.
