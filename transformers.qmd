...
After self-attention, the transformer applies a position-wise feedforward network (FFN) to each token independently — adding non-linearity and transforming the attended information further.
...
With these components — self-attention, feedforward networks, residual connections, and layer normalization — the encoder stack is complete. We now turn to the decoder, which builds on the encoder’s output to generate predictions.
...
In the decoder’s self-attention, it is critical to prevent the model from “looking into the future” — attending to tokens that come later in the sequence. This ensures proper causal sequence generation (left-to-right).
...
After applying the softmax, the model assigns non-zero attention only to tokens at positions ≤ $i$.
...
![Figure: Visualization of the attention mask applied to $QK^\top$, setting future positions to $-\infty$, and producing the masked attention matrix.](images/masked-attention.png){fig-align="center"}
...
5. Compute the weighted sum over the values to get the Cross-Attention Output $Z$:
$$
...
- Attends to the encoder’s hidden states through cross-attention, incorporating relevant information from the source sequence.
