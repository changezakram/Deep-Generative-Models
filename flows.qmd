...
**A Variational Autoencoder (VAE)** is a type of generative model that introduces latent variables $z$, allowing the model to learn compact, structured representations of the data. It is designed to support both sampling and likelihood estimation. Because the true marginal likelihood $p(x)$ is often intractable, VAEs use *variational inference* to approximate the posterior $p(z \mid x)$, and optimize a surrogate objective called the **Evidence Lower Bound (ELBO)**. This is made possible by the **reparameterization trick**, which allows gradients to flow through the latent variables during training.
