...
**Concept:**  
Test‑time scaling (TTS) enhances LLM performance during inference without modifying the base model’s parameters. Instead of retraining, TTS allocates more compute at inference — for example, by generating multiple candidate outputs, exploring reasoning paths, or dynamically adjusting computational resources based on task complexity. These methods can significantly improve reasoning, factual accuracy, and robustness while keeping the underlying model architecture unchanged.  

TTS is particularly valuable for scenarios where retraining is costly or infeasible, when inference budgets can be flexibly allocated, or when a base model already has sufficient competence but needs more careful reasoning or verification. From a cost perspective, TTS can achieve results comparable to much larger models on certain tasks (e.g., up to 14× improvement on intermediate‑difficulty benchmarks) while requiring only a fraction of the training compute.  


**Advantages:**
- Improves performance without retraining or additional data collection  
- Flexible — can be applied to any pre-trained model  
- Adaptable to task difficulty through dynamic compute allocation  
- Enhances multi‑step reasoning and factual accuracy via exploration and verification  
- Allows lightweight experimentation with different strategies

**Limitations:**
- Increases inference cost due to multiple generations or search steps  
- Benefits vary by task; may show diminishing returns on simple problems  
- Requires robust scoring or selection mechanisms (e.g., reward models, verifiers)  
- May add latency, especially for interactive applications  
- Performance gains depend on the quality of underlying base model
