...
|-----------------------------|---------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------|
| Instruction Finetuning      | Train LLMs to follow diverse instructions such as summarization, classification, QA, and creative writing. Enables zero-/few-shot generalization across task types. | Improves generalization and alignment. Makes outputs more helpful and controllable.                      | Requires large, curated datasets. Open-ended tasks are hard to evaluate. Can reflect human bias.      |
| Dialogue Finetuning         | Enable coherent multi-turn interactions for conversational agents like chatbots and digital assistants.                   | Improves coherence, context tracking, and user experience in dialogue systems.                            | Can overfit to chatty behavior. Needs large high-quality dialogue corpora.                            |
| Chain-of-Thought (CoT)      | Encourage step-by-step reasoning in tasks like math problems, logic puzzles, and commonsense QA.                         | Improves multi-step reasoning and interpretability.                                                       | Requires structured traces. Limited to reasoning-style tasks.                                         |
| Tool-Augmented Finetuning   | Teach the model to invoke external tools (e.g., calculators, search engines) and use their outputs.                       | Enables use of external capabilities, improving performance on complex or tool-supported tasks.           | Depends on reliable tool integration. Tool failures or latency affect output.                         |
| RAG Finetuning              | Combine retrieval with generation to improve factual accuracy in knowledge-intensive tasks.                              | Improves factual grounding and reduces hallucination.                                                     | Sensitive to retrieval quality. Risk of context injection errors.                                     |
| Alignment-Oriented Finetuning | Align model behavior with human values before reinforcement learning stages; improve safety and helpfulness.          | Promotes helpfulness and safety. Complements downstream RLHF.                                             | Limited by scope of value-alignment data. May encode narrow definitions of safety.                    |
| Long-Context Finetuning     | Extend model capacity to process and recall long documents, dialogues, or multi-part instructions.                       | Enhances memory and consistency across longer input contexts.                                             | Training is expensive. Long-context data is difficult to source.                                      |
| Mixture-of-Experts (MoE)    | Train models to use expert subnetworks selectively for efficiency and specialization.                                    | Improves scalability and efficiency. Reduces compute by activating only parts of the model.               | Training instability and routing issues. May lead to expert collapse.                                 |
| PEFT (e.g., LoRA, Adapters) | Allow efficient domain-specific tuning without updating the full model (e.g., via LoRA or adapters).                     | Resource-efficient. Enables fast adaptation with fewer parameters.                                        | May underperform full finetuning. Sensitive to hyperparameters.                                       |
| Multi-Objective Finetuning  | Jointly optimize for multiple goals (e.g., accuracy, safety, diversity) in the finetuning phase.                         | Balances competing priorities. Supports general-purpose tuning.                                           | Complex to design and evaluate. Hard to tune trade-offs across objectives.                            |
