...
The technique works by adding small adapter matrices that run in parallel to the frozen base weights. During training, the base model stays unchanged while only the adapters learn task-specific patterns. The outputs from both paths combine to produce specialized predictions. After training, the adapters can be merged back into the base weights, resulting in a single efficient model with no runtime overhead. 
