...
**2: Action Layer:** The Action Layer executes core banking work: loan origination, payments, fraud checks, cash management, and servicing. Domain Agents orchestrate these processes, connecting multiple systems and enforcing business rules. What used to be rigid, sequential workflows become policy-governed sequences that adapt based on data and context—within defined policy boundaries. This layer handles predefined business processes where the path is known; dynamic coordination across multiple agents is handled by the Intelligence Layer (Layer 3).
