---
format: html
title: "Building the AI-First Bank"
subtitle: "Agents, Architecture, and the Path to Autonomous Banking"
toc: true
---
...
![The AI-First Bank Architecture is organized into six functional layers. Experience Agents handle customer interaction, Domain Agents execute business processes, and the Intelligence Layer coordinates workflows—all wrapped in a unified Control Plane for governance.](images/ai_first_bank_architecture.png){#fig-Architecture fig-align="center" width="800"}
...
**2: Action Layer:** The Action Layer executes core banking work: loan origination, payments, fraud checks, cash management, and servicing. Domain Agents execute these processes, connecting multiple systems and enforcing business rules. What used to be rigid, sequential workflows become policy-governed sequences that adapt based on data and context—within defined policy boundaries. This layer handles predefined business processes where the path is known; dynamic coordination across multiple agents is handled by the Intelligence Layer (Layer 3).

**3: Intelligence Layer:** The Intelligence Layer governs and coordinates all AI agent activity across the bank. It manages model deployment, monitors performance, controls access to data, and enforces risk boundaries. This is where L1, L2, and L3 agents are orchestrated—ensuring that agents have access only to the data and systems appropriate to their role. For complex goals requiring multiple agents, the Intelligence Layer also handles planning—decomposing objectives into sub-tasks and dynamically coordinating the appropriate agents to execute them. The layer also handles prompt management, output validation, and model risk controls that regulators expect. Think of this as the coordination layer that connects modern agents with existing core systems—allowing banks to adopt agentic AI without replacing core platforms.
...
![L2 mortgage underwriting workflow showing experience agents handling intake, domain agents generating draft underwriting outputs, intelligence-layer coordination enforcing policy and risk controls, and human underwriters and credit officers approving final decisions.](images/Mortgage_Underwriting_Agents_.png){#fig-Mortgage-Underwriting fig-align="center" width="800"}
...
![The strategic progression of agentic banking. While Smart Overlays deliver immediate efficiency gains with low technical debt, true Reinvention requires deeper integration and fundamental process redesign.](images/AgenticBankingPathways.png){#fig-Pathways fig-align="center" width="800"}
...
Consider compliance. A compliance team already using an existing transaction monitoring platform can add AI agents on top—scanning flagged alerts, reducing false positives, and prioritizing cases for investigator review—without replacing the underlying system or changing how investigators work.

This works best when banks have clearly documented standard operating procedures. The AI agent follows these documented steps, ensuring consistency and compliance while adding intelligence. The benefit is speed: banks unlock near-term productivity gains without large-scale system replacements. Banks typically start with targeted deployments and expand Smart Overlay across a full business domain—such as Consumer Lending—as confidence and capability grow.

**Pathway 2: Agentic by Design**

Some processes can't be optimized by overlaying intelligence—they need to be rebuilt from the ground up. The Agentic by Design approach creates new, purpose-built autonomous applications tailored to specific banking functions. This approach is appropriate when existing systems cannot support the speed, intelligence, or regulatory precision the business requires—and when the process is central enough to justify the investment.

Think of it like microservices. Instead of one monolithic system, banks develop smaller, specialized agentic services that handle specific functions yet integrate smoothly into the broader infrastructure. For example, rather than replacing a 30-year-old loan origination system, a bank might build a new autonomous credit decisioning service that coordinates multiple agents: one for document verification, another for creditworthiness analysis, and a third for regulatory compliance checks.

This approach requires more upfront investment but delivers greater long-term flexibility. As regulations change or customer expectations evolve, individual agent services can be updated or replaced without disrupting the entire technology stack. Agentic by Design replaces legacy technology—not the operating model. How humans and processes interact remains largely intact; what changes is the system executing the work.

**Pathway 3: Process Redesign**

The boldest approach—and the hardest—involves fundamentally transforming the operating model, not just the technology. Unlike Pathway 2, which replaces legacy systems with purpose-built agentic services, Process Redesign changes how humans, agents, and workflows interact at their core. This pathway is appropriate when the current operating model itself is the constraint—not just the systems running it.

Traditional commercial credit review assumes a relationship manager manually gathers financial statements, extracts key financial data, and coordinates risk assessments across multiple teams—a process that can take weeks. Process redesign changes that assumption entirely: autonomous agents continuously monitor borrower financials, covenant compliance, and market conditions, proactively flagging deteriorating credits and triggering review workflows before risk escalates. The process isn't just faster—it's fundamentally different.
...
Most banks won't pursue all three approaches simultaneously. Smart overlays deliver quick wins in well-documented, repeatable processes. Agentic by Design delivers competitive differentiation where legacy systems are the primary constraint. Process redesign earns returns where business value is demonstrably high.
...
Above this layer, enterprise AI platforms such as ServiceNow orchestrate agents across systems and vendors from a central governance layer, while products like Salesforce Agentforce deliver pre-built agents for customer service and relationship management functions.

Finally, banking-specific platforms from providers like Temenos and Finastra embed pre-built domain expertise—covering lending, payments, compliance, and trade finance—directly into core workflows, reducing the time and cost of deploying AI in regulated environments.
...
## The AI-First Bank: Where Banking Is Headed
