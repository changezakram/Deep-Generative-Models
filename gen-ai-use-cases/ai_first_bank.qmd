...
The core risk is **behavioral drift**. Unlike static rule-based systems, agents optimize their actions dynamically based on feedback loops and outcome data. Research by Jiang et al. (2025) demonstrates that when agents and their supporting tools adapt simultaneously without coordination, systems can enter unstable "Red Queen" dynamics—continuous change without improvement. Without explicit controls regulating the pace of adaptation, these systems can develop unintended behaviors over time.

Banks must also guard against **optimization misalignment**, where an agent successfully hits a narrow metric (closing AML alerts quickly) while undermining the actual business goal (accurate fraud detection). This happens when targets are too specific and governance is too hands-off.
...
![**Figure:** The strategic progression of agentic banking. While Smart Overlays deliver immediate efficiency gains with low technical debt, true Reinvention requires deeper integration and fundamental process redesign.](images/Three-Pathways-to-Agentic-Banking.png){fig-align="center" width="800"}
...
**6. Organize for Federated Development.** As AI deployments scale, banks face a coordination challenge: centralized AI development creates bottlenecks, but complete decentralization creates incompatible systems.

BlackRock's Aladdin Copilot demonstrates the solution. Their central AI team established a standardized communication protocol and plug-in registry. Business units develop specialized agents for their domains—portfolio management, risk analysis, compliance—but all agents communicate through a common foundation. Credit teams build credit agents, risk teams build risk agents, each moving at their own pace, while central standards ensure interoperability.<sup>4</sup>
