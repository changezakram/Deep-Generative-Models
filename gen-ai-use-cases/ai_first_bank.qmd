...
![**Figure:** L3 agents operate autonomously in continuous observe-plan-act loops. They monitor environments, make decisions, execute actions, and adapt based on outcomes—all within governance boundaries. Unlike L2, humans are notified after decisions rather than approving before.](images/L-3.png){fig-align="center" width="800"}
...
Recent MIT Sloan research backs up what successful banks are learning: sustainable AI value comes from "small t" transformations—incremental improvements that build capability step by step—not sweeping enterprise redesigns that attempt to transform everything at once.

The L1 → L2 → L3 progression reflects this reality. Banks that spend time mastering L1 insight agents develop the governance muscle, technical skills, and organizational confidence they need before moving to L2 process orchestration. Those that skip ahead to L3 autonomous decisioning without that foundation tend to hit the same problems: failed pilots, compliance issues, teams that resist the technology.
...
A mature vendor ecosystem has emerged across three distinct layers. At the foundation, platforms such as Amazon Bedrock, Microsoft Azure AI, and Google Vertex AI provide model hosting, orchestration, and compute—commodity capabilities where custom development rarely creates advantage. Above that, cross-vendor orchestration tools like ServiceNow’s AI Platform coordinate agents across systems, while solutions such as Salesforce Agentforce offer pre-built agents for CRM and compliance use cases. Finally, banking-specific platforms from providers like Temenos and Finastra embed domain expertise directly into core workflows.
