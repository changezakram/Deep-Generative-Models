...
Reference-based evaluation is only as reliable as the references themselves. If the reference is flawed—due to poor quality, weak alignment, or narrow scope—it can mislead metrics and penalize valid outputs.

#### Case Study: CNN/Daily Mail Summarization  
This widely-used benchmark illustrates the reference quality problem in real-world evaluation. The CNN/Daily Mail summarization dataset was created by collecting news articles and using the bullet-point highlights written by journalists as "gold standard" summaries. These highlights were then used to train and evaluate summarization models.
 
**Problems Discovered:**
Several issues were identified with these reference summaries. Since the highlights were often written by journalists under time pressure, they tended to be incomplete, poorly structured, or inconsistent. Many of them emphasized surface-level details rather than content that users actually found summary-worthy. In some cases, the references didn't even reflect what end-users valued or needed from a summary.

**Experimental Evidence:**  
To assess the impact of reference quality, researchers conducted an evaluation comparing model outputs against two types of references:

1. **Original references**: the CNN bullet-point highlights  
2. **Expert references**: carefully crafted summaries written by trained annotators 

The results were striking. When using the original references, ROUGE scores showed **near-zero correlation** with human quality ratings. In contrast, scores computed against expert-written references demonstrated a **strong positive correlation** with human evaluations (r = 0.6–0.7). 

**Implication:**  
The evaluation metric wasn’t broken—the reference was. Poor references undermine the reliability of evaluation metrics like ROUGE.

#### Broader Challenges with Reference-Based Evaluation

Even beyond CNN/Daily Mail, reference quality issues are widespread across NLP benchmarks. Many tasks provide only a **single reference** per example, despite the existence of many equally valid outputs. This limits the diversity of acceptable responses and penalizes models for generating creative or paraphrased alternatives.

Additionally, references often exhibit **inconsistencies in tone, verbosity, or writing style**, which can bias metrics like BLEU or ROUGE that rely on surface-level matching. In some cases, reference summaries may highlight information that is **misaligned with the task objective**, especially in domains like creative generation or abstractive summarization. Moreover, crowdsourced references or those produced under time constraints may include grammatical errors, ambiguity, or shallow reasoning—adding **noise** that further degrades evaluation quality.
...
[8] Specia, L., et al. (2010). Quality Estimation for Machine Translation Without Human-Labeled Data. https://www.aclweb.org/anthology/W10-1753.pdf

[9] Su, Y., et al. (2023). Reference-Free Evaluation Metrics for Text Generation: A Survey. https://arxiv.org/pdf/2305.04648.pdf

[10] Celikyilmaz, A., et al. (2020). Evaluation of Text Generation: A Survey. https://arxiv.org/abs/2006.14799

[11] Krishna, K., et al. (2023). LongEval: Guidelines for Human Evaluation of Faithfulness in Long-form Summarization. https://aclanthology.org/2023.eacl-main.121.pdf

[12] Huyen, C. (2024). *AI Engineering: Building Applications with Foundation Models*. O’Reilly Media.  
