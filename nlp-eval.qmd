...
![**Figure:** BERTScore pipeline and formulas. The metric computes contextual similarity between candidate and reference tokens using BERT embeddings and cosine similarity, followed by precision, recall, and F1 aggregation. *(Source: Zhang et al. (2020))*](images/bert-score.png){fig-align="center" width="800"}
...
BLEURT takes a different approach: instead of computing semantic similarity heuristically, it learns to predict human ratings using fine-tuned BERT models.

1. **Pre-training**: Initialized with BERT and trained on synthetically modified sentence pairs.  
2. **Fine-tuning**: Trained on human-labeled examples to mimic human preferences.  
3. **Scoring**: Outputs a scalar score (typically between -1 and 1) indicating semantic similarity and naturalness compared to a reference.

BLEURT captures nuances such as grammar, fluency, and subtle paraphrasing that traditional metrics often miss.
