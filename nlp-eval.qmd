...
Despite rapid advances in automatic metrics, **human judgment remains the gold standard** for evaluating open-ended tasks. Nuance, subjectivity, and user-centric preferences often elude automated scoring methods, especially in tasks like summarization, dialogue, and creative generation. Human evaluation remains essential for validating whether model outputs are helpful, coherent, accurate, and aligned with human expectations.
...
There are two primary approaches to scoring: **absolute scoring** and **comparative judgment**. Absolute scoring methods use Likert scales, where annotators rate outputs numerically across different dimensions—typically on a 1-to-5 or 1-to-7 scale. While this provides granular data, it can introduce subjectivity due to differences in annotators' interpretations of the scale.
...
A large-scale review of NLP papers from 2015 to 2020 found that **only 5%** of studies provided enough methodological detail to replicate the human evaluation process. Common issues included missing annotator guidelines, inconsistent scoring criteria, and the absence of publicly shared data. This undermines scientific rigor and prevents meaningful comparisons across models and studies.
...
Below is a sample rubric commonly used for evaluating summary quality:
