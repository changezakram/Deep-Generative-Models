...
Despite rapid advances in automatic metrics, human judgment remains the **gold standard**—especially for evaluating open-ended tasks where nuance, subjectivity, and user preference matter. Human evaluation plays a critical role in:

- Validating the correlation of automatic metrics with human perception  
- Evaluating subjective tasks (e.g., helpfulness, engagement, creativity)  
- Capturing user-centric preferences not easily quantified  
- Detecting subtle quality issues that metrics miss
...
Human evaluation uses both **absolute scoring** and **comparative judgment** approaches.

- **Likert Scales**  
  - Annotators rate responses on a 1–5 or 1–7 scale for each dimension  
  - *Advantage*: Provides direct quantitative quality measurement  
  - *Challenge*: Subjective scale interpretation leads to annotator variability  

- **Pairwise Comparisons & Rankings**  
  - Annotators compare two or more outputs and choose the better one  
  - *Advantage*: Easier and more intuitive than absolute scoring  
  - *Use case*: Model benchmarking, leaderboard construction  

- **Best–Worst Scaling**  
  - From a group of outputs, identify the best and worst examples  
  - *Advantage*: Improves reliability over simple pairwise choices  
  - *Application*: Fine-grained quality judgments across multiple models  

#### The Human Evaluation Crisis

Despite its importance, human evaluation is fraught with reproducibility and consistency challenges.

**Reproducibility Problem**  
A large-scale review of NLP papers from 2015–2020 found that **only ~5%** of human evaluation setups were fully reproducible. Key problems included:

- Missing annotator guidelines or task definitions  
- Inconsistent scoring procedures across studies  
- Lack of publicly available outputs or instructions

The result is a **replication crisis**: scores across papers are difficult to compare, and findings are hard to validate.


**Agreement Challenges** 

- **Inter-annotator Agreement**: Typically ~60–70%, even with guidelines  
- **Subjective Tasks**: Can fall to 40–50% (e.g., summarization, helpfulness)  
- *Example*: In summarization, experts disagreed 30–40% of the time on quality ratings  

**Cost, Scale, and Quality Control**  

- **Cost**:  
  - Professional annotators charge $15–30/hour  
  - Evaluating a single output can take 5–15 minutes  
  - Meaningful evaluations often require hundreds or thousands of examples  

- **Quality Control**:  
  - Requires thorough annotator training  
  - Needs consistent monitoring and calibration  
  - Annotators may interpret vague guidelines differently without enforcement  

- **Incentive Misalignment**:  
  - Annotators aim to maximize throughput  
  - Researchers seek careful, thoughtful evaluation  
  - Result: Annotators may rush, use heuristics, or reward outputs with superficial polish  

#### Best Practices for Human Evaluation

Well-designed human evaluation protocols can mitigate many of these issues. Recommended practices include:

- **Clear Instructions** – Detailed rubrics with examples and edge cases  
- **Pilot Studies** – Run small-scale pilots to refine the procedure  
- **Multiple Annotators** – Use 3–5 evaluators per item to improve reliability  
- **Quality Monitoring** – Track inter-annotator agreement over time  
- **Feedback Loops** – Conduct calibration sessions and clarify discrepancies  

#### Example Evaluation Rubric: Summary Quality

| Score | Description |
|-------|-------------|
| **5 – Excellent** | Captures all key points, accurate, well-written, and concise |
| **4 – Good** | Covers most content with minor clarity or factual issues |
| **3 – Fair** | Some important information missing or unclear |
| **2 – Poor** | Major gaps in coverage or significant factual errors |
| **1 – Unacceptable** | Incoherent or completely fails to address the input |

> **Note**: Never compare human scores across studies—evaluation prompts, instructions, and annotators all differ.
...
Evaluating open-ended tasks is inherently complex. No single approach—automatic, human, or LLM-based—works universally well across all use cases.

Traditional metrics like BLEU and ROUGE remain useful, especially for structured tasks like translation, but they’re increasingly insufficient for tasks involving reasoning or generation. Human evaluations, while more insightful, are expensive, slow, and often irreproducible. Meanwhile, LLM-based evaluation is fast, scalable, and increasingly well-aligned with human judgments—but not without limitations.

In practice, the best evaluation strategy is hybrid: use multiple metrics, validate with human feedback, and always manually inspect outputs. Numbers alone can be misleading.
...
[18] Huyen, C. (2024). *AI Engineering: Building Applications with Foundation Models*. O’Reilly Media.  
