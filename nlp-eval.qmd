...
Understanding how to measure performance is fundamental to closed-ended evaluation. Different metrics serve different purposes and can lead to very different conclusions about model quality

**Task-Specific Metrics:** These metrics evaluate performance on individual labeled tasks and require ground-truth answers:
...
**Language Modeling Metric (Perplexity):** Perplexity measures how "surprised" a model is by text. Lower perplexity is better for predicting next words. 

$$
\text{Perplexity} = 2^{-\frac{1}{N} \sum_{i=1}^{N} \log_2 p(x_i)}
$$

For each word in a text, the model predicts how likely that word is. Perplexity averages these predictions across all words. If the model consistently gives high probability to the correct words, perplexity is low. If the model is often surprised (gives low probability to correct words), perplexity is high.
