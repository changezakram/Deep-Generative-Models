...
Taking the gradient of a log function follows this identity:
...
This is a **weighted sum** of the gradients \( \nabla_\theta f_\theta(x) \), where the weight of each \( x \) is proportional to how likely it is under the model.

Since the model defines:

$$
p_\theta(x) = \frac{\exp(f_\theta(x))}{Z(\theta)}
$$

we can rewrite the gradient of the log partition function as an **expectation**:
