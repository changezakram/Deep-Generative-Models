...
This ability to compare inputs without computing the partition function opens up a variety of use cases — from anomaly detection to denoising. But EBMs can do more than assess single samples. They can also **combine multiple expert models** to make joint decisions, enabling powerful forms of compositional reasoning.

This brings us to one of the most impactful capabilities of EBMs — compositional generation through the **Product of Experts (PoE)** framework.

### Product of Experts (Compositional Generation)

Sometimes we want to combine multiple expert models, each scoring an input $\mathbf{x}$ for different properties — for example, age, gender, or hairstyle. This is where Energy-Based Models (EBMs) shine through **Product of Experts (PoE)**.

Suppose you have three trained models $q_{\theta_1}(x)$, $r_{\theta_2}(x)$, and $t_{\theta_3}(x)$. A tempting idea is to combine their scores multiplicatively:

$$
p_{\theta_1}(x) \, q_{\theta_2}(x) \, r_{\theta_3}(x)
$$
...
In the figure below (Du et al., 2020), EBMs were used to model attributes such as "young", "female", "smiling", and "wavy hair". Combining them via Product of Experts allowed for flexible generation of images with **conjunctive constraints**:

![Product of Experts Face Generation](images/product_of_experts_faces.png)

> **Source:** Du et al., 2020. [*Compositional Visual Generation with Energy Based Models*](https://arxiv.org/pdf/2004.06030)
