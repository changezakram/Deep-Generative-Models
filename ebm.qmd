...
\section*{Understanding the Probability Foundation Behind EBMs}

In generative modeling, a valid probability distribution \( p(x) \) must satisfy two conditions:

\begin{itemize}
  \item \textbf{Non-negativity:} \( p(x) \geq 0 \)
  \item \textbf{Normalization:} \( \int p(x)\,dx = 1 \)
\end{itemize}

Directly defining a function that satisfies both is often difficult, especially when using flexible models like neural networks.

\subsection*{Why Introduce \( g(x) \)?}

Instead of modeling \( p(x) \) directly, we break the problem into two steps:

\begin{enumerate}
  \item Define an unnormalized, non-negative function \( g(x) \geq 0 \)
  \item Normalize it by computing \( Z = \int g(x)\,dx \), and define:
  \[
  p(x) = \frac{g(x)}{Z}
  \]
\end{enumerate}

This trick ensures we get a valid probability distribution while keeping the modeling process flexible and expressive.

\subsection*{Why This Works}

\begin{itemize}
  \item \( g(x) \) can be any non-negative function — for example, \( g(x) = \exp(f(x)) \)
  \item This makes it easy to parameterize \( f(x) \) using neural networks
  \item The normalization constant \( Z \) handles the requirement that \( p(x) \) sums to 1
\end{itemize}

\subsection*{Connecting to EBMs}
...
We define energy as \( E_\theta(x) = -f_\theta(x) \), so the probability becomes:

\[
p_\theta(x) = \frac{e^{-E_\theta(x)}}{Z(\theta)}
\]

This approach is flexible, expressive, and closely tied to the exponential family of distributions.
