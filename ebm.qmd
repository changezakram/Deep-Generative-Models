...
The challenge with sampling from an energy-based model is that we define:

$$
p_\theta(x) = \frac{1}{Z(\theta)} \exp(f_\theta(x))
$$

Unlike autoregressive or flow-based models, there’s **no direct way to sample** from this distribution, because computing the normalizing constant $Z(\theta)$ is intractable. However, we can **still compare scores** of different $x$ — which allows us to build samples iteratively.

## Markov Chain Monte Carlo (MCMC)

The core idea behind MCMC is to construct a Markov chain whose stationary distribution is $p_\theta(x)$. If we run the chain long enough, we can sample approximately from the target distribution.

### 1. Metropolis-Hastings (MH)

A classic MCMC method. At each step:
...
2. Accept the new sample with probability:
   ...
   $$
   \min\left(1, \exp(f_\theta(x') - f_\theta(x^t)) \right)
   $$
...
### 3. Adjusted Langevin MCMC

Like ULA, but applies a Metropolis-Hastings correction step after the Langevin update to ensure correct stationary distribution.
...
HMC treats sampling as a physics simulation. It introduces a momentum variable and simulates Hamiltonian dynamics to explore space more efficiently.

**Pros:**  
- Much better mixing in high-dimensional settings  
- Can take large steps while maintaining accuracy

**Cons:**  
- Requires tuning step size and number of leapfrog steps  
