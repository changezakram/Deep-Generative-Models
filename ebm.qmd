...
2. **Repeat** for $t = 0, 1, 2, \dots, T - 1$:  
   ...
   - Update: $x^{t+1} = x^t + \epsilon \nabla_x \log p_\theta(x)\big|_{x = x^t} + \sqrt{2\epsilon} z^t$  
...
Adjusted Langevin MCMC applies a **Metropolis-Hastings correction** step after each Langevin update to ensure samples follow the correct stationary distribution.

This makes it a **theoretically sound** version of ULA.

- **Update proposal**: Same as ULA  
  $$
  x^{t+1} = x^t + \epsilon \nabla_x \log p_\theta(x^t) + \sqrt{2\epsilon} z^t,\quad z^t \sim \mathcal{N}(0, I)
  $$

- **Accept or reject** the proposal using:
  $$
  \text{Accept } x^{t+1} \text{ with probability } \min\left(1, \frac{p_\theta(x^{t+1})}{p_\theta(x^t)}\right)
  $$

**Pros**  
- Theoretically correct: guarantees convergence to $p_\theta(x)$  
- More robust than ULA in many settings  

**Cons**  
- Requires rejection sampling, which slows down sampling  
- More expensive per iteration than ULA  
...
Hamiltonian Monte Carlo treats sampling as a **physics simulation** using Hamiltonian dynamics. It introduces a **momentum variable** to simulate trajectories and explore the space more efficiently.

At each step:
- Simulate dynamics using gradients of the log density $f_\theta(x)$  
- Use **leapfrog integration** to update position and momentum  
- Apply Metropolis-Hastings accept/reject step

**Pros**  
- Excellent mixing in high dimensions  
- Can take large, informed steps without losing accuracy  

**Cons**  
- Requires tuning (step size, number of leapfrog steps)  
- Each step is expensive (multiple gradients per sample)  
- Less common in EBMs due to high per-step cost  
