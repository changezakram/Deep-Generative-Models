...
To build intuition, consider how EBMs model the compatibility between variables. In the example below, the energy function assigns lower values where \( x \) and \( y \) are more likely to occur together, and higher energy elsewhere.

![Energy Field](images/energy_field.png)

<p class="caption" style="text-align: center; font-size: 0.85em; color: #666;">
  Visualizing the energy landscape $E(x_1, x_2)$ — lower energy corresponds to higher probability.
</p>

::: {.callout-note appearance="simple"}
Energy functions create **landscapes** over input space — where real data sits in valleys and noise sits on peaks.
:::
---

## 2.4 From Scores to Probabilities in EBMs

Energy-Based Models (EBMs) follow the same foundational idea we've seen before: define a **scoring function** $f_\theta(x)$ that assigns high values to likely data points, then convert those scores into probabilities using exponentiation and normalization.

This approach allows us to build **flexible probabilistic models** without requiring invertibility or tractable likelihoods.

### From Scores to Unnormalized Probabilities

We begin by using the scoring function $f_\theta(x)$ to define an unnormalized probability score:

$$
g_\theta(x) = \exp(f_\theta(x))
$$

This ensures $g_\theta(x) \geq 0$ and allows us to interpret $f_\theta(x)$ as an unnormalized log-probability.

To turn this into a valid probability distribution, we normalize using the **partition function** $Z(\theta)$:

$$
p_\theta(x) = \frac{g_\theta(x)}{Z(\theta)} = \frac{\exp(f_\theta(x))}{Z(\theta)} \quad \text{where} \quad Z(\theta) = \int \exp(f_\theta(x)) \, dx
$$

This gives us a valid probability distribution $p_\theta(x)$ even if $f_\theta(x)$ is defined by a neural network or other flexible function.

### Introducing Energy

Many EBMs reframe this using an **energy function** $E_\theta(x)$, where:

$$
E_\theta(x) = -f_\theta(x)
$$

Substituting this into our equation gives the classic EBM form:

$$
p_\theta(x) = \frac{\exp(-E_\theta(x))}{Z(\theta)}
$$

In this view, **lower energy means higher probability**.

---

### Why Use the Exponential?

We use the exponential function for three key reasons:

- **Non-negativity**: ensures $\exp(f_\theta(x)) \geq 0$
- **Interpretability**: allows $f_\theta(x)$ to act like a log-probability
- **Generality**: connects naturally to known distributions (e.g., Boltzmann, exponential family)
...
- Normalize with the **partition function** $Z(\theta)$

This gives EBMs the freedom to model complex data distributions using any differentiable function for $f_\theta(x)$ — without the constraints of invertibility or exact likelihoods.
--- 
