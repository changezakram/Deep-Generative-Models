...
Langevin MCMC improves sampling by incorporating gradient information.
...
**Pros**:
- Uses gradient to guide sampling
- Faster mixing than vanilla MCMC

**Cons**:
- Still computationally expensive
- Convergence slows in high dimensions

---

### 3. Summary of Trade-offs

| Method               | Gradient Use | Pros                                    | Cons                                            |
|----------------------|--------------|-----------------------------------------|-------------------------------------------------|
| Metropolis-Hastings  | ❌ No         | General, proven convergence              | Slow mixing, inefficient in high dimensions     |
| Unadjusted Langevin  | ✅ Yes        | Gradient-guided, faster mixing           | Still costly, can suffer from discretization    |

::: {.callout-tip appearance="simple"}
**Key Insight**  
MCMC allows us to sample from EBMs even when we can’t normalize $p_\\theta(x)$. But these methods are **slow** and **scale poorly** in high dimensions — especially when needed for every training step.
:::
