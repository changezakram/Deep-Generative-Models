...
### Understanding the Gradient of the Partition Function

To train EBMs, we often need to compute the gradient of the log partition function:

$$
\nabla_\theta \log Z(\theta)
$$

We know the partition function is:

$$
Z(\theta) = \int \exp(f_\theta(x))\, dx
$$

Let’s compute its gradient. Using the chain rule for logs:
...
This expression tells us that we are taking a **weighted average** of the gradients \( \nabla_\theta f_\theta(x) \), where each gradient is weighted by how likely \( x \) is under the model — proportional to \( \exp(f_\theta(x)) \).
...
So we can rewrite the gradient more cleanly as:

$$
\nabla_\theta \log Z(\theta) = \mathbb{E}_{x \sim p_\theta} \left[ \nabla_\theta f_\theta(x) \right]
$$

---

### Connecting to Likelihood Gradient

Recall that the log-likelihood of training data is:
...
Substituting the result we just derived:
...
- The first term increases the score of the actual training point — we want it to be more likely.
- The second term **averages the score gradients** over everything the model currently thinks is likely.
- The model learns by **increasing the score of the correct data** and **decreasing the score of samples it mistakenly believes are likely**.

This is the core principle behind training Energy-Based Models: pull up the data you want, push down the rest.
---

### Intuition

This expectation means we’re averaging the gradient \( \nabla_\theta f_\theta(x) \), but not over arbitrary data — only over the inputs that the model currently believes are likely (i.e., sampled from \( p_\theta(x) \)).

So we can interpret this gradient as:

> The average direction in which the model wants to increase the scores of samples it already thinks are likely.

This explains why we must “push down” the scores for these high-probability samples during training to reduce \( Z(\theta) \) and improve contrast.
