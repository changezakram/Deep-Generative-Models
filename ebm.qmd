...
It’s easy to define functions that satisfy non-negativity (e.g., using exponentials), but ensuring that the function integrates to 1 — especially when using neural networks — is much harder.

---

### Why Do We Introduce $g(x)$?

Instead of modeling $p(x)$ directly, we define a flexible, non-negative scoring function $g(x) \geq 0$, and normalize it to obtain a valid probability distribution:
...
- $g_\theta(x)$ provides the unnormalized score  
- $Z(\theta)$ ensures the output forms a proper probability distribution  
...
In Energy-Based Models, we define the unnormalized score using an energy function:

$$
g_\theta(x) = \exp(-E_\theta(x))
$$

This gives us the EBM form:

$$
p_\theta(x) = \frac{\exp(-E_\theta(x))}{Z(\theta)}
$$

Here, $E_\theta(x)$ is the **energy**: lower energy = higher probability.

To provide a more intuitive interpretation, many implementations define a scoring function $f_\theta(x)$ and relate it to energy:

$$
E_\theta(x) = -f_\theta(x) \quad \Rightarrow \quad p_\theta(x) = \frac{\exp(f_\theta(x))}{Z(\theta)}
$$

In both cases, the core idea is the same:

- Use flexible models to assign **unnormalized scores**
