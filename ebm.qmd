...
The expectation $\mathbb{E}_{x \sim p_\theta} \left[ \nabla_\theta f_\theta(x) \right]$ involves **sampling from the model distribution** $p_\theta(x)$, which requires computing or approximating the partition function $Z(\theta)$. This is computationally expensive and often infeasible in practice.
