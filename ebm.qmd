...
Training EBMs involves computing the gradient of the log partition function:
...
We start with the definition of the partition function:

$$
Z(\theta) = \int \exp(f_\theta(x))\, dx
$$

Taking the gradient of the log of this quantity using the chain rule:
...
This is a **weighted average** of gradients \( \nabla_\theta f_\theta(x) \), where each weight is proportional to how likely \( x \) is under the model.

Using the definition of the model distribution:

$$
p_\theta(x) = \frac{\exp(f_\theta(x))}{Z(\theta)}
$$

we can simplify the gradient to:
...
### Connecting to the Log-Likelihood Gradient

In training, our objective is to **maximize the log-likelihood** of training data:
...
- **Pull up the training example**: The first term increases the score of the training data.
- **Push down other samples**: The second term is an average over model samples — it reduces the scores of other points the model currently believes are likely.
- **Net effect**: Make the training data stand out as more likely than the rest.

This balance is what makes EBM learning effective: boost the good, suppress the rest.
