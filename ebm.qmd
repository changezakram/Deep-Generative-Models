...
1. **Learn a scoring function** (energy) where:  
   - Assigns low energy to real data (`x ∼ p_data`) 
   - Assigns high energy to noise

2. **Using generic neural nets**—no specialized architectures needed   

This approach provides unique advantages:  
✓ **Simpler setup** - works with any neural net
✓ **Explicit scoring** - $p(x) \propto e^{-E(x)}$ (but $Z$ is tricky)  
...
To understand how this works mathematically, we must first answer:  
**What defines a valid probability distribution?**  
