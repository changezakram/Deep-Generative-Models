...
To train an EBM, we want to increase the probability of training examples. So our objective is to **maximize the log-likelihood** of a training point $x_{\text{train}}$:

$$
\log p_\theta(x_{\text{train}}) = \log \left( \frac{\exp(f_\theta(x_{\text{train}}))}{Z(\theta)} \right)
$$
...
Now, we compute the gradient of this expression with respect to the model parameters $\theta$:
...
Taking the gradient of the **log partition function**:
...
$$
= \frac{1}{Z(\theta)} \nabla_\theta \left( \int \exp(f_\theta(x)) \, dx \right)
$$
...
Now observe that this is the **expected value** of $\nabla_\theta f_\theta(x)$ under the model distribution $p_\theta(x)$:
