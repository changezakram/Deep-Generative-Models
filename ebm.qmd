...
To train EBMs, we often need the gradient of the log partition function:
...
Now recall the derivative rule for logs:
...
This is a **weighted sum** over all possible values of \( x \), where each \( \nabla_\theta f_\theta(x) \) is weighted by how likely \( x \) is according to the model — specifically by \( \exp(f_\theta(x)) \).
...
That weighted sum turns out to be the **average of the gradients** — but not over just any inputs. We're averaging over inputs that the **model believes are likely**, because they're sampled from \( p_\theta(x) \).

This lets us interpret the gradient of the log partition function as:

> “How much, *on average*, the model wants to increase the score of the samples it already believes in.”
...
- The **first term** increases the score of the training point.
- The **second term** reduces the score of typical samples under the model.
- This contrast between **training data vs. generated data** is the key idea behind contrastive divergence.

In practice, the expectation $\mathbb{E}_{x \sim p_\theta}[\cdot]$ is approximated using samples drawn from the model (e.g., via MCMC).
