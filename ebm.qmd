...
The key idea of MCMC is to construct a Markov chain whose stationary distribution is  
$begin:math:text$ p_\\theta(x) $end:math:text$. This lets us obtain samples by running the chain long enough.
...
2. Accept $begin:math:text$ x' $end:math:text$ with probability  
   $begin:math:text$ \\min\\left(1, \\exp(f_\\theta(x') - f_\\theta(x^t)) \\right) $end:math:text$
...
**Pros**:  
- Leverages gradients to move toward likely regions  
- More efficient than random proposals

**Cons**:  
- No MH correction → approximate only  
