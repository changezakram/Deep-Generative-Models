...
1. **Learning a scoring function** (energy) where it assigns low energy to real data ($x \sim p_{data}$), and high energy to noise.

2. **Linking to probability** via: $p_\theta(x) = \frac{e^{-E_\theta(x)}}{Z(\theta)}, \quad Z(\theta) = \int e^{-E_\theta(x)}\,dx$ where low energy = high probability
