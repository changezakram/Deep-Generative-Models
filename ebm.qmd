...
2. **Repeat** for $t = 0, 1, 2, \dots, T - 1$:  
   - Sample $z^t \sim \mathcal{N}(0, I)$  
   - Proposal: $x' = x^t + \epsilon \nabla_x \log p_\theta(x^t) + \sqrt{2\epsilon} z^t$
   - Reverse proposal: $q(x^t \mid x') = \mathcal{N}\left(x^t \mid x' + \epsilon \nabla_x \log p_\theta(x'), 2\epsilon I\right)$
   - Accept with probability $\alpha = \min\left(1, \frac{p_\theta(x') \cdot q(x^t \mid x')}{p_\theta(x^t) \cdot q(x' \mid x^t)}\right)$, set $x^{t+1} = x'$  
     - Otherwise, set $x^{t+1} = x^t$  
